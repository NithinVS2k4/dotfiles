# Generated by Powerlevel10k configuration wizard on 2026-05-24 at 00:07 IST.
...
# round separators, blurred heads, round tails, 2 lines, solid, full frame,
...
() {
  ...
  # Default current directory foregroundcolor.
  ...
  typeset -g POWERLEVEL9K_STATUS_OK_BACKGROUND=$POWERLEVEL9K_DIR_BACKGROUND
...
function prompt_my_command_execution_time() {
  local elapsed_ns=$(( int(P9K_COMMAND_DURATION_SECONDS * 1000000000) ))
  local out=""

  (( elapsed_ns > 0 )) || return

  if (( elapsed_ns >= 3600000000000 )); then
    local h=$(( elapsed_ns / 3600000000000 ))
    local m=$(( (elapsed_ns / 60000000000) % 60 ))
    local s=$(( (elapsed_ns / 1000000000) % 60 ))
    out="${h}h ${m}m ${s}s"

  elif (( elapsed_ns >= 60000000000 )); then
    local m=$(( elapsed_ns / 60000000000 ))
    local s=$(( (elapsed_ns / 1000000000) % 60 ))
    out="${m}m ${s}s"

  elif (( elapsed_ns >= 1000000000 )); then
    local s=$(( elapsed_ns / 1000000000 ))
    local ms=$(( (elapsed_ns / 1000000) % 1000 ))
    out="${s}s ${ms}ms"

  elif (( elapsed_ns >= 1000000 )); then
    local ms=$(( elapsed_ns / 1000000 ))
    local us=$(( (elapsed_ns / 1000) % 1000 ))
    out="${ms}ms ${us}μs"

  elif (( elapsed_ns >= 1000 )); then
    local us=$(( elapsed_ns / 1000 ))
    local ns=$(( elapsed_ns % 1000 ))
    out="${us}μs ${ns}ns"
  ...
  p10k segment \
    -f "${POWERLEVEL9K_COMMAND_EXECUTION_TIME_FOREGROUND}" \
    -b "${POWERLEVEL9K_COMMAND_EXECUTION_TIME_BACKGROUND}" \
    -i '' \
    -t "$out"
}
typeset -g POWERLEVEL9K_MY_COMMAND_EXECUTION_TIME_FOREGROUND=$POWERLEVEL9K_COMMAND_EXECUTION_TIME_FOREGROUND
typeset -g POWERLEVEL9K_MY_COMMAND_EXECUTION_TIME_BACKGROUND=$POWERLEVEL9K_COMMAND_EXECUTION_TIME_BACKGROUND
